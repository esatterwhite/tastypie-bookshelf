...
var tastypie    = require('tastypie')
  ...
  , debug       = require('debug')('tastypie:resource:bookshelf')
...
// make sure the fields get registerd
require('./../fields/related');
...
ApiField.prototype.db_prop = function( table ){
  return ( table || this.resource.options.queryset.prototype.tableName ) + '.' + this.options.attribute;
};
...
BookshelfResource = Resource.extend({
    ...
    ,full_dehydrate: function( obj, bundle, done ){
        obj = ( obj && obj.toJSON ) ? obj.toJSON() : obj;
        return this.parent( 'full_dehydrate', obj, bundle, done.bind( this ) );
    }

    /**
     * Responsible for converting a raw data object into a resource mapped object
     * @method module:tastypie-bookshelf/lib/resource/bookshelf#full_hydrate
     * @param {module:tastypie/lib/resource~Bundle} bundle
     * @return {module:tastypie/lib/resource~Bundle}
     **/
    , full_hydrate: function full_hydrate( bundle, done ){
        ...
        async.forEachOf( this.fields, function(field, fieldname, cb ){
            ...
            related_name = field.options.related_name || field.options.name;
            ...
            if( attr ){
                field.hydrate( bundle, function( err, value ){
                    if( field.is_related ){
                        bundle.object.set(attr, value ? value.get( field.instance.options.queryset.prototype.idAttribute ) : value );
                        // bookshelf doesn't do this check before serializing and will die w/out a model
                        ( value && value.toJSON ) && set( bundle.object, `relations.${related_name}`, value );
                    } else {
                        isFunction(bundle.object.set) ? bundle.object.set( attr, value ) : set( bundle.object, attr, value );
                    }
    ...
    , limit: function limit( queryset , bundle ){
        ...
        qs.limit = hasOwn(qs, 'limit' ) ? parseInt( qs.limit, 10 ) : qs.limit;
    ...
    ,get_objects: function get_objects( bundle, opts ){
        ...
        return new Promise(function( resolve, reject ){
          ...
          collection = that.options.queryset.collection().query(function( qb ){
          ...
          query_promise = collection
            .query((qb)=>{
                  ...
                  DEBUG_SQL && debug( qb.toQuery() );
            })
            .fetch( that.options.prefetch ? {withRelated:that.options.prefetch} : null);


          Promise.all([count_promise, query_promise ])
            .then(( results )=>{
                resolve( {count:values( results[0][0] )[0], objects:results[1].toJSON() });
            }).catch(( err )=>{
                err.res = bundle.res;
                err.req = bundle.req;
                err.name = err.code;
    ...
    ,get_object: function get_object( bundle, cb ){
        let Model = this.options.queryset
          , params = {}
          ;
    ...
    ,create_object: function create_object( bundle, cb ){
        var that;

        that = this;
        bundle.object = new that.options.queryset();
        that.full_hydrate( bundle, function( err, bndl ){
          if( err ){
              err = annotate( err, bundle );
              return that.exception( err );
          }
          bundle.object
                  .save()
                  .then(function( obj ){
        ...
        this.get_object( bundle, function( err, obj ){
            if( err || !obj ){
                if( err ){
                  ...
                  err = Boom.notFound( `no ${that.options.name} found with id ${bundle.req.params.pk}` );
                }
                return that.exception(err);
            }
            bundle.object = obj;
            that.full_hydrate(bundle, function( err, bndl ){
                ...
                bndl.object.save( bundle.object.toJSON({shallow:true}) )
                           .then( function( o ){
                                bundle.object = bundle.object.toJSON ? bundle.object.toJSON() : bundle.object;
    ...
    , buildJoins:function( qb, bits, filter ){
      ...
      current_model = current.options.queryset.prototype;
      related_model = related && related.options.queryset.prototype;
